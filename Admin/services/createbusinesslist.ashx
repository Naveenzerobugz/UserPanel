                    ...
                    if (context.Request.Files.Count > 0)
                    {
                        ...
                        for (int i = 0; i < files.Count; i++)
                        {
                            ...
                            if (file.ContentLength <= 4457280 && isValidExt)
                            {
                                ...
                                int r = Business.InserBusinessList(title, tagline,category, address,"India",city,state, zipcode, mob, email,website,fb,linkdin,twitter, googleplus,description,minprice,maxprice,video, "/upload/"+path, DateTime.Now);
                                switch (r)
                                {
                                    case 1:
